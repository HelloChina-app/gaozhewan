...
2. 用户可扫描页面二维码填写收款地址，或手工复制地址；二维码只编码 Base58Check 收款地址，币种、网络和金额仍由用户在钱包中确认。
3. 用户提交邮箱和 64 位 TxID；服务端只查询 `only_confirmed=true` 的链上事件。
4. 同时确认：事件来自 Tether 官方 USDT 合约、类型为 `Transfer`、收款地址完全匹配、到账金额不少于标价。
5. 服务端以 TxID 为不可覆盖路径写入 Private Blob；重复 TxID 只能由第一次登记的邮箱哈希重试，不能用于第二个账号。
6. 核验通过后立即返回一年期签名访问地址，用户点击即写入 HttpOnly Cookie 并进入完整选题工作台。
